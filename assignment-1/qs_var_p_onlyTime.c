...
void main()
{
    ...
    FILE *fout;
    fout = fopen("quicksort_time_variation_for_different_problem_ set ", "w");
    fprintf(fout, "QuickSort time variation file for diff. size prblm set");
    fprintf(fout, "\n");
    fprintf (fout,"the number of iterations per prblm set size is %d",iter_num);
    fprintf(fout, "\n");
    ...
    for (int p = 0; p < 4; p++)
    {
        ...
        printf("avg. time taken for per problem set size of 2^(%d) is %0.4f ms\n", p, (((end_time - start_time) * 1000 / CLOCKS_PER_SEC)) / iter_num);
        printf("\n");
        fprintf(fout, "total time taken for problem set size of 2^(%d) is %0.8f ms\n", p, (end_time - start_time) * 1000 / CLOCKS_PER_SEC);
        fprintf(fout, "avg. time taken for per problem set size of 2^(%d) is %0.8f ms\n", p, (((end_time - start_time) * 1000 / CLOCKS_PER_SEC)) / iter_num);
        fprintf(fout, "\n");
    }
    double end_time = clock();
    printf("\n");
    printf("total time taken for different size dataset is %0.8f ms\n", (end_time - start_time) * 1000 / CLOCKS_PER_SEC);
    printf("\n");
    fprintf(fout, "\n");
    fprintf(fout, "total time taken for different size dataset is %0.8f ms\n", (end_time - start_time) * 1000 / CLOCKS_PER_SEC);
    fprintf(fout, "\n");
}